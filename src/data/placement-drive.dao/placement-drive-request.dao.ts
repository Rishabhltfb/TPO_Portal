...
export default class PlacementDriveRequestDAO {
  ...
  async getAllUnapprovedPlacementDriveRequests(): Promise<Array<PlacementDriveRequest>> {
    const requests: Array<PlacementDriveRequest> = await PlacementDriveRequestModel.find({ status: 'OPEN' });
    return requests;
  }
}
