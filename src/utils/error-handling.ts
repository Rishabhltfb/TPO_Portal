...
export function NotFoundExceptionHandler(
  ...
  return res.status(404).send(responseAdapter.sendErrorResponse(error.message, 404));
}
...
export function GenericExceptionHandler(
  ...
  return res.status(errCode).send(responseAdapter.sendErrorResponse(error.message, errCode));
}
