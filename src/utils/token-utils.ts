...
export default class TokenUtils {
  ...
  private REFRESH_TOKEN_SECRET_KEY: string = process.env.REFRESH_TOKEN_SECRET_KEY || '';

  private JWT_TOKEN_EXPIRY_TIME_IN_MINTUES = process.env.JWT_TOKEN_EXPIRY_TIME_IN_MINTUES;

  async createJwtToken(tokenPayload: AuthTokenPayload) {
    return jwt.sign(tokenPayload, this.JWT_SECRET_KEY, {
      expiresIn:
        process.env.NODE_ENV !== 'production' && this.JWT_TOKEN_EXPIRY_TIME_IN_MINTUES
  ...
  async verifyRefreshToken(refreshToken: string, ignoreExpiration?: boolean): Promise<AuthTokenPayload> {
    try {
      const payload = (await jwt.verify(refreshToken, this.REFRESH_TOKEN_SECRET_KEY, {
        ignoreExpiration: ignoreExpiration !== undefined ? ignoreExpiration : false,
      })) as JwtPayload;
      return {
